...
function ScoreRow({ label, zh, score, onChange, tone }: { label: string; zh: string; score: number; onChange: (v: number) => void; tone: string }) {
  ...
  return <section className="score-row">
    <div className="score-heading"><div className={`score-label ${tone}`}>{label}<small>{zh}</small></div>
...
export default function Home() {
  ...
  return <main ref={pageRef}><div className="topbar"><span>TOEIC® SCORE ESTIMATOR</span><span>多益答對題數計算器</span></div><div className="page-shell">
    ...
    <section className="score-card"><div className="score-main"><ScoreRow label="LISTENING" zh="聽力" score={listeningScore} onChange={setListeningScore} tone="orange"/><ScoreRow label="READING" zh="閱讀" score={readingScore} onChange={setReadingScore} tone="orange"/></div><aside className="total-score"><span>TOTAL SCORE<small>總分</small></span><b>{listeningScore + readingScore}</b><em>/ 990</em></aside></section>
